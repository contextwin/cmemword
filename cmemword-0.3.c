...
/*	function	*/
void WaitUserInput(WINDOW *win, const char *format, const void *variable_p) {
...
//		getmaxyx(stdscr, h, w);
		...
		wrefresh(question_text_wnd);
                ...
                WaitUserInput(user_input_wnd, "%hd", &user_input_num);
		...
		for (;;) {
			...
			wrefresh(question_text_wnd);
			WaitUserInput(user_input_wnd, "%hu", &user_input_num);
		...
		for (;;) {
			...
			wrefresh(question_text_wnd);
			WaitUserInput(user_input_wnd, "%c", &user_input_y_or_n);
			...
			if ('y' == user_input_y_or_n) {
				...
				if ((1 == user_input_num) || (3 == user_input_num) || (4 == user_input_num)) {
					...
					for (;;) {
						...
						wrefresh(question_text_wnd);
						WaitUserInput(user_input_wnd, "%hd", &number_of_end_question);
						...
						if (number_of_end_question > question_max) {
							...
							mvwaddstr(user_input_wnd, 1, 0, "A number greater than the actual number of file was entered.");
							wrefresh(user_input_wnd);
							WaitUserInput(user_input_wnd, "%c", &user_input_num);
				...
				wrefresh(user_input_wnd);
				WaitUserInput(user_input_wnd, "%c", user_input_answer);
		...
		for (;;) {
			...
			wclear(question_text_wnd);
			wprintw(question_text_wnd, "出題が終わりました。プログラムを終了しますか?\n");
			wprintw(question_text_wnd, "r でもう一度同じ出題を繰り返します。(y/n/r)\n");
			wrefresh(question_text_wnd);
			WaitUserInput(user_input_wnd, "%c", &user_input_y_or_n);
			...
			if ('y' == user_input_y_or_n) {
				...
				wclear(question_text_wnd);
				wprintw(question_text_wnd, "y(yes) か n(no) か r(retry) を入力して下さい。\n\n");
				wrefresh(question_text_wnd);
			}
		}
